...
//*************************************** BASE DE DATOS WEBSQL
	...
	var urlP = "https://roinet.pe/demo/index.php/mobile_controller/";
...
//*********************************************** FIN BASE DE DATOS WEBSQL
	...
	function evaluarProspecto(id, va)
	{
		...
		$.ajax({
			...
			success : function(data) {
				if(data == "ok"){
					if(va == "g"){ alert("Prospecto Ganado"); $.mobile.back();}
					else { alert("Prospecto Perdido"); $.mobile.back();}
					llenarFase();
					llenarProspecto(us);
					llenarCuenta(us);
					llenarContacto(us);
					...
					alert("Faltan llenar algunos requisitos en la oportunidad.")
				}else{
	...
	$("#nom_deci, #nece, #fecha_aprox, #sig_cita, #hra_cita").keyup(function(){
		if( $(this).val() != "" ){
			$(".menError").fadeOut();			
			return false;
		}		
	...
	function editProspectoOff(id)
	{
		...
		var pr = "UPDATE tb_prospecto SET necesidad='"+$('#nece').val()+"', propuesta='"+$('#prop').val()+"', fecha_aprox='"+$('#fecha_aprox').val()+"', cambio = '"+camPrE+"' WHERE id ="+$('#id_pros').val();
		db.transaction(function(transaction) {
